...
Partial Class Secure_CollectInfo
    ...
    Protected Sub btnGetfile_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnGetfile.Click
        Try
            Using connection As New SqlConnection(ConfigurationManager.ConnectionStrings("SpeechServices").ConnectionString)
                ...
                cmd.Parameters.AddWithValue("@File", Convert.ToString(FileUpload1.FileName))
    ...
    Protected Sub btnUplaod_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnUplaod.Click
        Try
            If FileUpload1.HasFile AndAlso FileUpload1.FileBytes.Length > 0 Then
                Dim s As String
                s = System.Text.Encoding.Default.GetString(FileUpload1.FileBytes).Substring(0, IIf(FileUpload1.FileBytes.Length > 8000, 8000, FileUpload1.FileBytes.Length))
                txtContent.Text = Regex.Replace(s, "[^\u0000-\u007F]", "")

                'txtContent.Text = Encoding.ASCII.GetString(
                'Encoding.Convert(
                'Encoding.UTF8,
